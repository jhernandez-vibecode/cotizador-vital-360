...
### Fuentes v1.3 (10 sep 2026 · `4d28cd9`) — la letra de marca SDI
**Google Sans Flex** (títulos y texto; titulares ≥24 px a peso **500**, nombres 600) +
**Google Sans Code** (cifras secundarias, códigos, pólizas). La familia de Google Ads, libre
(OFL) desde nov 2025. Vive en el `<link>` del `<head>` y en las variables `--font-display`
/ `--font-body` / `--font-mono` / **`--font-big`** del bloque LÍNEA CLARA (index) y
`--f-disp` / `--f-body` / `--f-mono` (guía). Manual y archivos: skill `imagen-de-marca-sdi`
+ `SDI-BRAND-KIT/linea-clara/`. Esto deroga las dos pilas anteriores (Hanken "Camino B" del
21 jul y Space Grotesk + Inter + JetBrains Mono del 9 sep) — no reinstalarlas.
- **La cifra protagonista (`.v7-rec-tot .big`) y el monto de "Elija el ritmo"
  (`.v7-pago-now .amt`) van en `--font-big` = Google Sans Flex tabular**, como en Google
  Ads. Las demás cifras siguen en `--font-mono`.
- 🔴 **Google Sans Flex NO trae el ₡ (U+20A1). Inter va segunda en la pila y en el `<link>`
  SOLO para prestarlo** (el navegador baja únicamente su subconjunto latin-ext). Quitar Inter
  = el ₡ cae a la fuente del sistema. Verificado con zoom a 2× y en móvil real de 375 px:
  `scrollWidth == clientWidth`, Flex/Code cargadas, Inter latin-ext cargada.
- Los **correos no cambian** (`_LC_F` Arial / `_LC_M` Courier New): Gmail no carga webfonts.
- 🔴 **El pane del navegador sirvió la cara VIEJA (navy, Hanken) desde caché** al verificar,
  aunque `http-server -c-1` manda no-cache: una entrada cacheada por heurística en visitas
  anteriores no se revalida. Ante "veo lo viejo", navegar con `?bust=<n>&c=…` o capturar en
  headless con perfil limpio.

### El ₡ de Space Grotesk se montaba sobre el dígito (histórico, ya no aplica)
El glifo U+20A1 de Space Grotesk es más ancho que su avance: `₡1.877.216` salía con el
símbolo pisando el 1, con cualquier `letter-spacing`. Por eso del 9 al 10 sep la cifra
grande fue monoespaciada. Con la v1.3 el ₡ lo presta Inter y la protagonista volvió a la
letra de títulos. Arial tiene el mismo defecto en el correo: ahí las cifras siguen en
`Courier New`.

### Filete SDI en el encabezado (11 sep 2026)
Pedido de JC ("faltó el filete de colores SDI"; se hizo primero en INS Medical y luego acá). Línea
de 4 px con los 4 colores de la barra del logo del kit (`#0369A1` · `#0D9488` · `#EA580C` ·
`#C9A227`, tomados del SVG). Vive al final del `<style>` como `--lc-filete`, aplicado con `::before`
a `.masthead-agente` (consola), `.hero-top` (cliente) y `.lc-solo` (login). **En los tres correos lo
pone `_barraCorreoLC`** (fila de 4 celdas de 25 % antes de la barra): tocar esa pieza = tocar los
tres. Verificado en localhost con `getComputedStyle(…, "::before")` y en producción.
...
### Los correos · A `67e6a2e` (9 sep) · B y C `0dc38af` (10 sep)
**Los tres en línea clara y en producción.** Comparten cinco piezas: `_barraCorreoLC`,
`_reglaCorreoLC`, `_estadoCorreoLC`, `_botonCorreoLC` y `_pieCorreoLC` (constantes
`_LC_F` / `_LC_M`). **Tocar el pie de los correos = tocar `_pieCorreoLC`**: ya no hay tres
copias a mano. Siguen existiendo los dos pies de la PÁGINA (agente y cliente), así que
la regla "al tocar la leyenda, tocar todos" sigue en pie para esos dos + esta pieza.
- **El texto se TRANSCRIBIÓ, no se redactó** (memoria `integrar-modulo-existente`). En la B
  se verificaron **30 de 30** frases legales y operativas idénticas a la versión anterior:
  plazos, correos del INS, los 6 teléfonos, Ley 8956 art. 16, revocatoria/apelación,
  documentos del reembolso. Si alguien "mejora" la redacción de la B, está tocando texto
  legal aprobado.
- **Decisión de JC (10 sep): variante CLARA de la B.** Se descartó dejar "Números y correos
  clave" en navy y "Sus derechos" en ámbar. La caja de teléfonos es la **única caja** de su
  zona y se encuentra por aislamiento; documentos adicionales y derechos van como texto
  abierto con su regla de 28×3. No reproponer la variante en color.
- Las 4 modalidades de la B **perdieron su color propio** (verde/teal/coral/navy): número en
  círculo neutro. JC rechaza los "chips de icono de color".
- La píldora de estado de B y C dice "Póliza emitida y vigente", **no la fecha**: la fecha ya
  está en la tarjeta de la póliza, dos centímetros abajo.
- Al refactorizar la A para usar las piezas, su salida quedó **idéntica a la publicada**
  (comparada sin comentarios ni espacios). Método reutilizable: guardar la salida de la
  plantilla con un payload FIJO antes de tocarla y comparar después.
...
#### Fichas uniformes en la A (14 sep 2026 · `3d705db`) — SUPERSEDE al bloque siguiente
JC mandó la captura de las tarjetas de la página y pidió "la plantilla del correo se deben ver así también las fichas de las opciones". Ahora el correo usa **la misma ficha que `.v7-card`** y la misma regla de filas: 1-3 en una fila, 4 en 2×2, 5-6 de a 3, 7 de a 4 (4+3; en 4 columnas el ámbito se acorta a "Local / Regional / Internacional" y baja un punto la letra).
- 🔴 **Cada ficha ES una celda `<td>` de la fila**, no una tabla anidada: así todas las fichas de una fila miden lo mismo de alto (una tabla anidada no se estira). Separadores de 10 px como `<td>` vacíos y una fila de 12 px entre filas. La ficha de la opción a valorar lleva borde azul de 2 px y la píldora azul con estrella; **todas reservan la fila de la píldora** (un `&nbsp;` del mismo alto) para alinearse.
- La opción a valorar **ya no va ancha ni lleva el desglose adentro**: el aporte por asegurado va en una tarjeta aparte debajo de la rejilla ("Cuánto aporta cada uno / usted en X, la opción a valorar"). Con 1 plan no hay píldora (como en la página).
- Verificado renderizando `buildEmailVital360` con 7, 3 (USD, 1 asegurado) y 1 planes en Chrome headless: el `?c=` largo sigue en el cuerpo (regla #11), 0 "recomiend", fichas de igual alto por fila. Método reutilizable: extraer el `<script>` de `index.html` a un `app.js` en el scratchpad y cargarlo desde un `render-*.html` con `file://` + `--allow-file-access-from-files` (`window.onerror=()=>true` para que el `init()` sin DOM no rompa).
- **Smoke real de JC en Gmail: PENDIENTE** (el del 11 sep cubría la versión anterior).

#### Tarjetas en la A (11 sep 2026 · `32d7440`) — histórico, superseded el 14 sep
JC pidió que el correo presente las opciones **como las tarjetas de la consola** (ámbito ·
nombre · "suma anual" · monto). Mockup con dos variantes → eligió la **1**: la opción propuesta
va ANCHA (borde azul de 2 px, píldora **"★ Opción a valorar"** con estrella `&#9733;` en
`#F4B400`, y el aporte por asegurado adentro) y las demás en **pares de 50 %**. Reemplazó
los dos bloques anteriores (panel del recomendado + tabla "Las N opciones") por uno solo.
- **La píldora ya NO dice "Lo que le recomiendo"**: JC la cambió a "Opción a valorar", tono
  más neutro. No volver al texto viejo.
- Con 1 plan no hay pares (`otras.length === 0`); con impar, la última celda es `&nbsp;`.
- Solo cara: `totalesPlan` / `desgloseMiembros` intactos, el `?c=` largo sigue en el cuerpo
...
eligió para el correo B (lo operativo se destaca por aislamiento, no por color).
- **Barra en dos niveles:** arriba el INS en tinta + "Llamar al 800-TELE-INS" (sigue fija);
  abajo "Vital 360 · Guía de reclamos" y anclas `#situaciones` `#documentos` `#contactos`
  `#agente` (ids NUEVOS; los del JS no se tocaron).
- **Escenarios:** el número sale por `counter` de CSS y oculta el emoji del ícono — no hubo
  que tocar el HTML de las 6 tarjetas. **Avisos:** banda pálida + un punto de color; el rojo
  solo donde el original era `alert-red` (pre-autorización ANTES, LLAME ANTES).
- **Documentos:** los 2 contratos a lo ancho (`a:nth-of-type(-n+2)`), 6 formularios en rejilla.
  🔴 `#doc-particulares` se estila por **ID, no por `.featured`**: `cpSinDocumento()` le quita
  la clase y, si fuera por clase, la rejilla quedaba con un hueco.
- Los `!important` del bloque son SOLO contra estilos en línea del HTML (fondos de color de
  íconos y etiquetas) y contra los que escribe `cpSinDocumento()`. No agregar más.
- 🔴 **Bug cerrado:** el pie traía `Licencia SUGESE 08-1318` escrito a mano. `personalizar()`
  llena la licencia en la tarjeta del agente (`#g-agente-lic`) pero nunca tocó el pie: en la
  guía de OTRO agente la página decía 99-0000 arriba y 08-1318 abajo. Verificado con la
  versión vieja y en producción con el payload de otro agente (0 apariciones). **La licencia
  vive solo en la tarjeta del agente; no volver a escribirla en el pie.**
- De paso: `.step-content strong{display:block}` partía en su renglón las negritas DENTRO de
  una frase — ahora `.step-content p strong{display:inline}`.
- Texto: de 202 renglones solo cambiaron la barra, los enlaces, las dos píldoras del inicio,
  el título "Su agente" y el pie. El botón de emergencia pasó de "🆘 800-TELE-INS" a
  "Llamar al 800-TELE-INS" (se le avisó a JC).
...
- **Tipografía (desde 10 sep 2026, marca SDI v1.3):** Google Sans Flex (títulos y texto) · Google Sans Code (cifras secundarias) · Inter solo para prestar el ₡. Ver "Línea clara › Fuentes v1.3". El `:root` viejo (línea ~30) todavía declara Hanken Grotesk / Spline Sans Mono, pero el bloque LÍNEA CLARA del final del `<style>` lo pisa; esas fuentes ya no están en el `<link>`. Histórico: IBM Plex (may) → Hanken "Camino B" (21 jul) → Space Grotesk + Inter + JetBrains Mono (9 sep) → v1.3.
...
- **Botón "✉ Correo" en cada fila (14 sep 2026 · `4678b60`)** — reenvía la COTIZACIÓN al cliente: abre `openEmailPreviewModal` con `buildEmailVital360(h.payload, h.link)`, o sea el mismo correo A del paso 3 armado desde el registro guardado, **sin tocar el `state` del cotizador** y conservando el `?c=` largo del registro (regla #11; si el registro no trae `link`, se reconstruye con `buildLinkCliente`). Sin email del cliente → alert. Historia: JC pidió "reenviar pólizas que ya había enviado" y se implementó primero el reenvío del correo de póliza vigente (`0cdb95b`); luego aclaró que lo que le faltaba era este botón. **Los dos existen ahora.** No marca ningún flag de "enviada" (no se pidió).
...
- **Reenviar póliza (14 sep 2026 · `0cdb95b`).** Pedido de JC: "necesito la opción de reenviar pólizas que ya había enviado". La pestaña gana el botón **📨 Enviar póliza / Reenviar póliza** (dorado; en modo reenviar, contorno) junto al de la guía, y la columna **"Correo póliza"** (Enviada + fecha / Pendiente), aparte de la columna "Guía".
  - `abrirEnvioPoliza(h)` es el **único camino** desde las dos pestañas (Mis cotizaciones también lo usa): exige el email del cliente y llama `openPolizaCaptureModal({ …, prefill: h.polizaData })`. Con `prefill`, el modal se titula "Reenviar la póliza vigente" y **viene lleno** con N.º, fecha, plan y moneda capturados; sin `polizaData` se comporta como siempre.
  - 🔴 **`marcarPolizaEnviada(id)` solo corre en el `onEnviado` de `enviarEmailDirecto`, después del 200 de Gmail.** "Copiar correo", "Abrir Gmail manualmente" y ".eml" NO marcan nada: el historial no puede decir "enviada" sobre un correo que no salió por la API. `onEnviado` es un parámetro opcional nuevo de `enviarEmailDirecto`; los otros dos modales no lo pasan.
  - Campos nuevos en el registro: `polizaEnviada` + `polizaEnviadaAt`. Los registros viejos (sin el campo) se ven "Pendiente" aunque el correo haya salido antes del 14 sep: el botón sigue diciendo "Enviar", y funciona igual. "Recuperar desde Gmail" NO reconstruye este flag (solo `polizaData` vía el asunto de la B).
  - Verificado en localhost con historial inyectado: columna y botones por fila, modal precargado (N.º, fecha, plan Pro 2, USD), preview al correo del cliente con el asunto correcto, segunda fila sin datos → modal vacío, flag persistido en localStorage, `runSmokeTests()` 0 fallos. **El envío real por Gmail no se probó** (queda para el smoke de JC).
...
Orden de secciones **real** (`renderClienteMain`, layout "v7" desde `3c547d0`, **reordenado el 14 sep 2026 en `167e30f`**; el listado viejo de cards horizontales + tabla de 28 desplegable **ya no existe**):
1. Hero blanco (línea clara) con logo INS + Vital 360 + datos del agente + **mapa de 3 pasos** (`.v7-ruta`, solo con varios planes)
2. Toggle moneda sticky (CRC/USD)
3. `renderClienteEscalera` — **Paso 1 · TODAS las opciones en tarjetas** (`.v7-cards`), incluida la opción a valorar con estrella; la tarjeta tocada abre su `renderPanelPlan` **adentro, a lo ancho** (regla crítica #12). Ver "Opciones primero" abajo.
4. `renderClienteRecomendado` — **Paso 2 · "Opción a valorar"** (ya no "Lo que le recomiendo"): total del año con IVA y **justificación calculada** contra el plan anterior y el siguiente
5. `renderClienteRecibo` — cuánto aporta cada asegurado (de la opción a valorar; en singular si es uno solo)
6. `renderClientePagoV7` — formas de pago: chips anual / semestral / trimestral (ver "Formas de pago en cada plan")
7. `renderClienteCobertura` — 9 coberturas clave + botón "Ver las 28" (`renderTabla28` con UN plan) + botón "Esquema INS" (lightbox con el JPG oficial según moneda)
8. `renderClienteComparador` — **Paso 3**: el CLIENTE marca hasta 3 planes y `renderTabla28` los pone lado a lado; **arranca con la opción a valorar y sus dos vecinas marcadas**
...
### Opciones primero · "Opción a valorar" (14 sep 2026 · `167e30f`)

**Qué lo provocó:** JC vio que la página entraba de una con la recomendación "y el cliente ni siquiera sabe sobre qué se lo estamos recomendando". Pidió mockup del flujo **Presentación → Opciones → Recomendación con el porqué → Comparativo**; se iteró 4 veces en el artifact y se pasó a la app tal cual. Mockup en disco: `mockup-flujo-opciones.html` (ignorado por git, como los otros dos).

- **Ya no existe la "recomendación" para el cliente: es la "Opción a valorar"**, igual que en el correo A (JC, 11 sep). Píldora con estrella `&#9733;` en `#F4B400` sobre el borde de la tarjeta y en el panel. No volver a "Lo que le recomiendo" ni a "Ver el plan que le recomiendo". Lo único que aún dice "Le recomiendo" es el **mensaje de WhatsApp** (`⭐ Le recomiendo …`, con 4+ planes): JC no lo pidió, se le avisó.
- **Las tarjetas son UNIFORMES y en filas completas** (orden de JC: "las 3 alineadas, no pueden quedar abajo a menos que sean 4"): 7 → 4+3, 6/5/3 → 3 por fila, 4 → 2×2, 2 → 2; ≤1000 px 2 columnas; ≤640 px una. Misma estructura en todas: ámbito · nombre · suma por persona · deducible · total con IVA · `prima + IVA` · enlace. La estrella va **montada sobre el borde** (`top:-12px`) para no cambiar la altura. Se quitaron los encabezados por ámbito (`.v7-grp`) y las filas `.v7-row` (CSS viejo queda, sin uso).
- **La tarjeta de la opción a valorar es un `<a href="#lc-plan">`**, no se "abre": manda al paso 2. Las demás son `<div class="v7-card"><button class="v7-card-btn" data-action="v7-plan">` + el panel adentro cuando `cli.planFoco === id`; la abierta toma `grid-column:1/-1`. Mismo handler `v7-plan` y mismo `scrollIntoView`, ahora sobre `.v7-card.open`.
- **Intro según asegurados:** `fam = p.asegurados.length > 1` → "N opciones para cuidar a su familia" / "para cuidar su salud", "para su familia" / "para usted", "Quiénes están cubiertos · Cuánto aporta cada uno" / "Quién está cubierto · Cuánto aporta usted". Con un solo plan: "Una opción…", sin paso 1, sin mapa y sin comparador (igual que antes).
- **Mapa de 3 pasos** (`.v7-ruta`) bajo los chips de la familia: círculos en tonos tenues del filete (azul / teal / naranja). Es una secuencia real, por eso lleva números. JC quitó de paso el "sin adjetivos" del paso 2.
- **Comparador preseleccionado** en `renderCliente`: `[anterior, rec, siguiente]` de `planesOrdenados()`.
- **Solo cara:** `totalesPlan`, `justificacionPlan`, `diferenciasCobertura`, `renderPanelPlan`, `renderTabla28` intactos. Verificado en localhost con clics reales (abrir/cerrar tarjeta, panel adentro con "Contra Pro 1" y 3 formas de pago, toggle USD recalculando tarjetas y panel), 7/3/1 planes × 4/1 asegurados, `runSmokeTests()` 0 fallos, 0 "recomiend" en el texto del cliente.
- 🔴 **Smoke de JC en teléfono real: PENDIENTE.** Lo que hay que mirar: que las tarjetas queden en una columna sin desborde, que al tocar una **se vea que se abrió ahí mismo** (regla #12), y que la estrella no tape el ámbito.
- Trampa de método: el pane del navegador oculto reporta `innerWidth 0` y no dibuja; las capturas se hicieron con Chrome headless (`--screenshot`, mínimo ~500 px de ancho, así que el móvil no se pudo capturar) y las interacciones con `javascript_tool` sobre el DOM.
...
**Precarga automática en la póliza vigente (10 sep 2026 · `88a6889`, pedido de JC).** El modal
`openEmailPolizaPreviewModal` llama `setupAdjuntosControl(modal, { precargar: formulariosINS })` y
adjunta solo **Solicitud de Beneficios, Autorización para Consulta de Expedientes y Condiciones
Generales** (1,8 MB, de `docs/`). Antes eran enlaces para bajar y volver a subir a mano, y el correo
ya prometía "(adjunto en este correo)". Reglas:
- La precarga pasa por el **mismo contador `leyendo`** → el botón de envío queda en "⏳ Cargando
  adjuntos…" hasta que terminan. No crear un camino paralelo que se salte ese contador.
- Si un `fetch` falla, **se avisa con alert** y el agente adjunta a mano: el correo le dice al cliente
  que van adjuntos. Nunca fallar en silencio.
- "Copiar correo" y "Abrir Gmail manualmente" **no llevan adjuntos** y los dos preguntan antes.
  Solo "Enviar ahora desde Gmail" y "Descargar .eml" los incluyen.
- El correo B dice "(adjunto en este correo)" en la Solicitud de Beneficios **y** en la Autorización.
...
1. **IVA 2 % — REGLA NUEVA (JC, 21 jul 2026): el IVA SÍ se suma y se muestra DESGLOSADO** (prima + IVA = total, ambos visibles para el cliente). Esto **revierte y deroga** la regla vieja "el IVA nunca se suma, solo es nota visual, sumarlo es bug crítico SUGESE" — **no reinstalarla.** `IVA_PCT = 0.02` (Ley 9635, seguros de personas). **Layout DEFINIDO el 27 jul 2026 — ya no hay nada que preguntarle:** el IVA se refleja en TODO cálculo que vea el cliente (incluido el desglose por miembro), la cifra grande es siempre el total con IVA y `prima + IVA` va como línea susurro debajo. Redondeo, excepción de WhatsApp y regla de cuotas: ver **Pendientes abiertos #1**, que trae la decisión completa. El comentario de `IVA_PCT` ya dice que la regla vieja quedó DEROGADA, y desde el 10 sep (`54ee093`) **el aviso legal del pie del cliente también lo dice** (antes afirmaba que los montos "no incluyen el IVA"). Verificado con grep: ningún texto al cliente contradice el IVA incluido.
...
- 🔴 **Smoke de JC en teléfono del reorden "opciones primero" (`167e30f`, 14 sep) — SIN HACER.** Ver "Vista cliente › Opciones primero". Se acumula a los dos siguientes (misma vista).
